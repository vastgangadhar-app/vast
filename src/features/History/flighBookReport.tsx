...
const FlightBookReport = () => {
  ...
  const recentTransactions = async () => {
    try {
      setLoading(true);
      const url = `${APP_URLS.FlightBookReport}pageindex=${present}&pagesize=500&txt_frm_date=2022-12-05&txt_to_date=${formattedDate}&ddl_status=${selectedFilter}&PNR=null`;
      console.log(url);

      const response = await post({ url: url });
      console.log('FlightBookReport',response);
